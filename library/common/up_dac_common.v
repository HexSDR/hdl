...
module up_dac_common #(
  ...
  output              dac_frame,
  output              dac_clksel,
  output     [24:0]   dac_data_control,
  output     [ 2:0]   dac_control,
  output              dac_par_type,
  output              dac_par_enb,
  output              dac_r1_mode,
  output              dac_datafmt,
  ...
  reg    [24:0]   up_dac_data_control = 'd0;
  reg    [ 2:0]   up_dac_control = 'd0;
  ...
  always @(negedge up_rstn or posedge up_clk) begin
    if (up_rstn == 0) begin
      ...
      if ((up_wreq_s == 1'b1) && (up_waddr[6:0] == 7'h19)) begin
        up_dac_data_control <= up_wdata[24:0];
      end
      if ((up_wreq_s == 1'b1) && (up_waddr[6:0] == 7'h1b)) begin
        up_dac_control <= up_wdata[2:0];
      end
    end
  end
    ...
    if (up_rstn == 0) begin
      ...
      if (up_rreq_s == 1'b1) begin
        case (up_raddr[6:0])
          ...
          7'h19: up_rdata_int <= {7'd0, up_dac_data_control};
          7'h1a: up_rdata_int <= {31'd0, up_sync_in_status};
          7'h1b: up_rdata_int <= {29'd0, up_dac_control};
