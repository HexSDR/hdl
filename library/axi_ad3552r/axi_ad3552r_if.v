...
wire [9:0]  ddr_transfer_data_0;
wire [9:0]  ddr_transfer_data_1;
wire [9:0]  ddr_transfer_data_2;
wire [9:0]  ddr_transfer_data_3;
...
assign cnt_value      = ( sdr_ddr_n == 1'b1) ? ((address_sent == 1'b1) ? 'h1f :'h2e) : 
...
always @(posedge clk_in) begin
 if (start_transfer == 1'b1 || (end_transfer_good == 1'b1 && valid_in_int==1'b1) ) begin 
  csn_int <= ~csn_int;
 end else begin
  csn_int <= csn_int;
 end 
...
always @(negedge clk_in) begin
  if ( start_transfer || valid_in_int == 1'b1 || cnt_enable_s == 1'b0 ) begin
    cnt_p <= 'h0;
  ...
  if (csn_int == 1'b1) begin
   end_transfer_good <= 1'b0;
   address_sent <= 1'b0;
  end 
  if( valid_in_int == 1'b1) begin 
    channel_sel <= ~channel_sel;
  end else if(csn_int == 1'b1) begin 
    channel_sel <= 1'b0;
  end 

  if( valid_in_int == 1'b1 || address_sent ==1'b0 ) begin 
...
assign transfer_data_0 = (sdr_ddr_n == 1'b1) ?  sdr_transfer_data_0 : {37'b0,ddr_transfer_data_0};
assign transfer_data_1 = (sdr_ddr_n == 1'b1) ?  sdr_transfer_data_1 : {37'b0,ddr_transfer_data_1};                                                     
assign transfer_data_2 = (sdr_ddr_n == 1'b1) ?  sdr_transfer_data_2 : {37'b0,ddr_transfer_data_2};
assign transfer_data_3 = (sdr_ddr_n == 1'b1) ?  sdr_transfer_data_3 : {37'b0,ddr_transfer_data_3};
...
assign clk_cnt_value = (sdr_ddr_n == 1'b1) ? 3'h3 : 3'h0;
assign sclk_int   = (csn_int == 1'b0) ? clk_div : 1'b0;

always @ (posedge clk_in) begin
  if (csn_int == 1'b1 || count == clk_cnt_value) begin
    count <= 4'b0;
    clk_div <= 1'b0;
